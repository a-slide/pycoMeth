...
def Meth_Comp (
    ...
    * pvalue_adj_alpha
    ...
    try:
        ...
        with tqdm (total=all_fp_len, desc="\t", unit=" bytes", unit_scale=True, disable=not progress) as pbar:
        ...
        with Comp_Writer(bed_fn=output_bed_fn, tsv_fn=output_tsv_fn, verbose=verbose) as writer:
            for res in tqdm(stats_results, desc="\t", unit=" sites", unit_scale=True, disable=not progress):
                writer.write (res)
...
class Comp_Writer():
    ...
    def __init__ (self, bed_fn=None, tsv_fn=None, verbose=True):
        ...
        self.bed_fn = bed_fn
        self.tsv_fn = tsv_fn
        self.bed_fp = self._init_bed ()
        self.tsv_fp = self._init_tsv ()
