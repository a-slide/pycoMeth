...
def Interval_Aggregate(
    ...
    * interval_bed_fn
        SORTED bed file containing **non-overlapping** intervals to bin CpG data into (Optional)
    * output_bed_fn
    ...
    * min_llr
    ...
    try:
        ...
        fp_out = Interval_Writer (
            ...
            verbose=verbose)
        try:
            with tqdm (total=len(fp_in), desc="\tInput file", unit=" bytes", unit_scale=True, disable=not progress) as pbar:
                # Get first line
                line = fp_in.next()
...
class Interval_Writer():
    ...
    def _write_bed (self, coord, med_llr):
        ...
        if med_llr >= self.min_llr+1:
            color = '225,0,0'
        elif med_llr >= self.min_llr:
            color = '255,170,0'
        elif med_llr <= -self.min_llr+1:
            color = '0,170,255'
        elif med_llr <= -self.min_llr:
            color = '0,0,225'
        else:
